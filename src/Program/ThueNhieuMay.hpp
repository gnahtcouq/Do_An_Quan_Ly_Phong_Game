...
class ThueNhieuMay {
   public:
    ...
    void themNodeVaoDau(ThueMotMay);            // thêm Node vào đầu
    ...
    void giaiPhongDanhSachNguoiThueTrucTiep();  // giải phóng danh sách người thuê trực tiếp
...
void sapXepDanhSachNguoiThueTheoSoMay_QuickSort(ThueNhieuMay &);      // sắp xếp danh sách người thuê theo số máy
...
void ThueNhieuMay::themNodeVaoDau(ThueMotMay thueMotMay) {
...
void sapXepDanhSachNguoiThueTheoSoMay_QuickSort(ThueNhieuMay &thueNhieuMay) {
    ThueNhieuMay t1, t2;
    Node *tag, *p;
    if (thueNhieuMay.pHead == thueNhieuMay.pTail)
        return;
    t1.taoListThueMotMay();
    t2.taoListThueMotMay();
    tag = thueNhieuMay.pHead;
    thueNhieuMay.pHead = thueNhieuMay.pHead->pNext;
    tag->pNext = NULL;
    while (thueNhieuMay.pHead != NULL) {
        p = thueNhieuMay.pHead;
        thueNhieuMay.pHead = thueNhieuMay.pHead->pNext;
        p->pNext = NULL;
        if (p->data.maytinh <= tag->data.maytinh)
            t1.themNodeVaoDau(p->data);
        else
            t2.themNodeVaoDau(p->data);
    }
    sapXepDanhSachNguoiThueTheoSoMay_QuickSort(t1);
    sapXepDanhSachNguoiThueTheoSoMay_QuickSort(t2);
    if (t1.pHead != NULL) {
        thueNhieuMay.pHead = t1.pHead;
        t1.pTail->pNext = tag;
    } else
        thueNhieuMay.pHead = tag;
    tag->pNext = t2.pHead;
    if (t2.pHead != NULL)
        thueNhieuMay.pTail = t2.pTail;
    else
        thueNhieuMay.pTail = tag;
}
...
void ThueNhieuMay::thanhToan(MayTinh nhieuMay[], int n) {
    if (n != 0) {
        ...
        if (kiemTra == true) {
            ...
            ghiHoaDon << "\n*---------------------- STU CYBER ----------------------*\n";
            ghiHoaDon << "\n*----------------- THÔNG TIN THANH TOÁN ----------------*\n";
            ghiHoaDon << setw(30) << left << "| Số máy: ";
            ghiHoaDon << setw(28) << right << r.maytinh->soMay << " |\n";
            ghiHoaDon << setw(30) << left << "| Kiểu máy: ";
            if (loaiKieuMay == "Thường")
                ghiHoaDon << setw(31) << right << loaiKieuMay << " |\n";
            else
                ghiHoaDon << setw(30) << right << loaiKieuMay << " |\n";
            ...
            ghiHoaDon << setw(30) << left << "| Thời gian kết thúc: ";
            ghiHoaDon << setw(30) << right << thoiGianSuDung << " |\n";
            ghiHoaDon << setw(30) << left << "| Số tiền cần thanh toán: ";
            ghiHoaDon << setw(29) << right << giaTien << " |";
            ghiHoaDon << "\n*-------------------------------------------------------*\n";
            ghiHoaDon << "\nSTU CYBER ghi nhận các đóng góp của quý khách nhằm\nnâng cao chất lượng dịch vụ thông qua";
